...
// ---------------------------------------------------------------------------
// Cayley graph visualizer with 3-phase node modes (normal -> drag -> pin).
// This file is the corrected, integrated version — fixes earlier issues with
// instance colors not updating and OrbitControls not being disabled during
// dragging. Paste this file over your existing one.
// ---------------------------------------------------------------------------
...
const MODE_COLORS = {
  normal: new THREE.Color(0x999999),
...
function InstancedNodes({
  ...
  useEffect(() => {
    ...
    if (meshRef.current && meshRef.current.material) {
      meshRef.current.material.color.set("#999999");
...
export default function CayleyGraphVisualizer() {
  ...
  function ensureInstanceColorAttribute(mesh) {
    ...
    if (!attr || attr.array.length < num * 3) {
      ...
      // initialize all entries to default gray (#999999)
      const gray = new THREE.Color("#999999");
      for (let i = 0; i < num; i++) {
        const base = i * 3;
        arr[base + 0] = gray.r;
        arr[base + 1] = gray.g;
